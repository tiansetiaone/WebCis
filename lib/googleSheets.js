...
export async function saveFeedbackToGoogleSheet(data) {
  const { email, document, relevance, stage, timestamp } = data;
  
  try {
    // Implementasi penyimpanan ke Google Sheets
    ...
    // Ganti dengan ID spreadsheet dan range yang sesuai
    const spreadsheetId = process.env.GOOGLE_SHEET_ID;
    const range = 'Feedback!A2:E'; // Sesuaikan dengan sheet Anda

    const values = [
      [timestamp, email, document, relevance, stage]
    ];
    ...
    console.log('Feedback saved successfully:', email);
    
  } catch (error) {
    console.error('Error saving feedback to sheet:', error);
    throw error;
  }
}
